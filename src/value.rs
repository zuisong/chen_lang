...
impl fmt::Display for RuntimeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            RuntimeError::TypeMismatch { expected, found, operation } => {
                write!(f, "Type mismatch in {}: expected {:?}, found {:?}", 
                       operation, expected, found)
            }
            RuntimeError::DivisionByZero => write!(f, "Division by zero"),
            RuntimeError::InvalidOperation { operator, left_type, right_type } => {
            ...
            RuntimeError::CallNonFunction(t) => write!(f, "Attempt to call non-function value: {:?}", t),
        }
    }
}
