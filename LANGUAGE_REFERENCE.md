...
- ❌ **`goto` 标签** - Lua 5.2+ 的 `goto`/`::label::` 语法
- ❌ **函数调用实参展开** - 除变长参数 `...` 外，`f(g())` 中 `g()` 的多返回值不会自动展开
- ⚠️ **数组索引** - 数组为 0-based（`{ "a" }` 索引从 0 开始），与 Lua 的 1-based 不同
- ⚠️ **`table.sort`** - 暂不支持自定义比较函数

> ✅ 闭包（捕获外部作用域变量）已完整支持。
>
> 详细的 Lua 兼容特性清单请参阅 [LUA_COMPAT.md](LUA_COMPAT.md)。
